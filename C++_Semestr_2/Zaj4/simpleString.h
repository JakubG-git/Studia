...
protected:
    ...
    size_t capacity_;
public:
    SimpleString() : data_(new char[1]), size_(0),  capacity_(0) {instances_++; data_[0] = 0;};
    SimpleString(const char* text);
    SimpleString(const SimpleString &string);
    ~SimpleString();
    bool equal_to(SimpleString text2) const;
    bool equal_to(SimpleString text2, bool yes_no) const;
